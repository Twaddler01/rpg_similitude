...
function load_game_from_text() {
    ...
    const importSaveBtn = create_el('importSaveBtn', 'button', e_load_JSON_text);
    importSaveBtn.innerHTML = 'Import SaveData';
    const savedata_section = create_el('savedata_section', 'div', e_load_JSON_text);
    ...
    importSaveBtn.addEventListener('click', () => {
        ...
        if (!import_save_container) {
            ...
            const ta_box = create_el('ta_box', 'textarea', import_save_container);
            ta_box.rows = 10;
            ta_box.style.width = '100%';
            const submit_import_save = create_el('submit_import_save', 'button', import_save_container);
            submit_import_save.innerHTML = 'SUBMIT';
            const reset_import_save = create_el('reset_import_save', 'button', import_save_container);
    ...
    const export_save = create_el('export_save', 'button', e_save_JSON);
